...
class Retrieve:
    ...
    def compute_document_vectors_and_length(self):
        ...
        # A dictionary containing lengths of documents. Key = doc_id. 
        # Value = length of doc.
        doc_lengths = {}
        for term in self.index:
            
            # Initialise list to store a document's vectors.
            lengths_array = []
            ...
            for doc_id in self.index[term]:
                ...
                # Attaches tf to the dictionary
                doc_vectors[doc_id][term] = freq
                    
                
                # Euclidean distance using numpy, more efficient
                lengths_array.append(freq)  
                l_a_np = np.array(lengths_array)
                doc_lengths[doc_id] = np.sqrt(np.sum(l_a_np*l_a_np))
    ...
    def for_query(self, query):
        ...
        query_vectors = self.compute_query_vectors_and_length(query)
        cosine_similarities = {}
        ...
        for doc_id in range(1, self.num_docs+1):
            ...
            length_doc_denominator = doc_lengths[doc_id]
